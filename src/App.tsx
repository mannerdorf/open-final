...
// 🛑 Удалена строка: import "./styles.css"; (для устранения ошибки разрешения пути, повторно)
...
// Функционал: Функция для скачивания файла через прокси
const downloadFile = async (auth: AuthData, cargo: CargoItem, metod: string): Promise<string | null> => {
    ...
    try {
        ...
        if (!response.ok) {
            let errorText = `Ошибка HTTP: ${response.status}`;
            try {
                // Пытаемся прочитать ответ как JSON для получения ошибки из прокси
                const errorJson: ApiError = await response.json();
                errorText = errorJson.error || errorText;
            } catch (e) {
...
const CargoCard = ({ cargo, onToggleDetails, onDownload }: { 
    ...
    const [isDownloading, setIsDownloading] = useState<'Счет' | 'Акт' | null>(null); // State: which button is loading

    // [МОДИФИКАЦИЯ 1: Локальная функция для управления состоянием загрузки]
    const handleDownloadClick = (metod: 'Счет' | 'Акт') => {
        setDownloadError(null);
        setIsDownloading(metod); // 1. Начинаем загрузку, показываем спиннер
        
        // 2. Делегируем вызов API родительскому компоненту
        onDownload(cargo, metod, (err) => {
             // 3. Callback вызывается по завершении API-запроса
             setIsDownloading(null); // Завершаем загрузку
             setDownloadError(err); // Устанавливаем ошибку (или null)
        });
    };
    // [КОНЕЦ МОДИФИКАЦИИ 1]
                ...
                <div className="card-details">
                    ...
                    <div className="download-container">
                         ...
                         <div className="download-buttons">
                            <button 
                                onClick={() => handleDownloadClick('Счет')} 
                            ...
                            <button 
                                onClick={() => handleDownloadClick('Акт')} 
                                className="download-button"
...
const CargoListDisplay = ({ cargoList, isLoading, error, refetch, auth }: { 
    ...
    // Удалены состояния downloadingState и downloadErrorState, так как CargoCard управляет ими локально.
    ...
    // [МОДИФИКАЦИЯ 2: Функция, которая вызывает downloadFile и сообщает результат обратно в CargoCard]
    const handleDownload = useCallback(async (cargo: CargoItem, metod: 'Счет' | 'Акт', callback: (error: string | null) => void) => {
        ...
        // 1. Выполняем фактическое скачивание с использованием утилиты и текущих данных аутентификации
        const error = await downloadFile(auth, cargo, metod); 
        
        // 2. Вызываем callback-функцию, чтобы обновить состояние в CargoCard
        callback(error);
            ...
            {localCargoList.map((cargo, index) => (
                <div key={index} className="mb-4">
                    <CargoCard 
                        ...
                        onDownload={handleDownload} // Передаем нашу новую функцию
                    />
                </div>
            ))}
        </div>
